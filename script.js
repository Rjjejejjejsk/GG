// script.js — оновлено під нові категорії та пошук за джерелом
...
function showAuthorPhrases(author) {
    renderResults(phrases.filter(p => p.author === author), `Фрази автора: ${author}`);
}
function showCategory(category) {
    renderResults(phrases.filter(p => p.category === category), `Категорія: ${category}`);
}

function renderResults(list, title) {
    const r = document.getElementById('results');
    if (!list.length) { r.innerHTML = '<p>Нічого не знайдено.</p>'; return; }
    r.innerHTML = `<h3>${title}</h3>` +
        list.map(p => `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('source-search-btn').onclick = () => {
        ...
        renderResults(
            phrases.filter(p => p.source.toLowerCase().includes(q)),
            `Результати пошуку за джерелом: "${q}"`
        );
    };
